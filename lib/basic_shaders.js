...
exports.OrenNayar = exports.basicShader = undefined;

var _shader = require('./shader');

var basicText = function basicText() {
    return new _shader.Shader('\
        varying vec2 vTex;\
...
var basicShaderDepth = function basicShaderDepth() {
    return new _shader.Shader('\
        varying vec4 vPosition;\
        void main(void) {\
        vPosition = gl_ModelViewMatrix * vec4(Vertex, 1.0);\
 ...
 *     });
 */
var basicShader = exports.basicShader = function basicShader(options) {
    var settings = {
        useBump: false,
        useDiffuse: false,
        useAtlas: false,
    ...
    var fragSource = '\
			...
			}' : '') + (settings.useLights ? 'vec3 lightPow(Light li,vec2 til){\
            ...
            vec3 returnedLight = vec3(0.0,0.0,0.0);\
				...
				bmpp = (bmpp -0.5) * 2.0;\
				dW = dW*bmpp.x*(material.bumpWeight)+dW*(1.0-material.bumpWeight);' : '') + 'if(dW>0.0){' + (settings.useSpecular ? '\
            ...
            " : "\
            tiler = vTex;") + (settings.useTiling ? 'tiler = vec2(vTex.s*tiling.x,vTex.t*tiling.y);\
            ' : '') + (settings.boxMapping ? '\
...
var OrenNayar = exports.OrenNayar = function OrenNayar(options) {
    var settings = {
        useBump: false,
        useDiffuse: false,
        useAtlas: false,
...
var basicMobileShader = function basicMobileShader(options) {
    var settings = {
        useBump: false,
        useDiffuse: false,
        useAtlas: false,
