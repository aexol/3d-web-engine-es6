...
var Resource = exports.Resource = function () {
    ...
    _createClass(Resource, null, [{
        key: "loadNextElement",
        value: function loadNextElement() {
        ...
        key: "loadFunction",
        value: function loadFunction(path, resp) {
            var request = new XMLHttpRequest();
            request.open("GET", path, true);
            var ext = path.split(".").pop();
            if (ext == "obj") {
                request.responseType = "arraybuffer";
            } else if (ext == "js" || ext == "json") {} else {
...
Resource.bin.rASCII0 = function (buff, off) {
    var s = "";
    while (buff[off] != 0) {
        s += String.fromCharCode(buff[off++]);
    }return s;
};
...
Resource.parse._buffToStr = function (buff) {
    ...
    for (var i = 0; i < a.length; i++) {
        s = s.concat(String.fromCharCode(a[i]));
    }return s;
};
...
Resource.parse._readLine = function (a,
// Uint8Array, offset
off) {
    var s = "";
    while (a[off] != 10) {
        s += String.fromCharCode(a[off++]);
    }return s;
};
    ...
    for (var g in groups) {
        ...
        for (var tt in triangles) {
            ...
            for (var i = 0; i < 3; i++) {
                ...
                var c = t[i][1];
                var n = t[i][2];
                var vmap = [vertices[v], n >= 0 && n < normals.length ? normals[n] : [0, 0, 0], c >= 0 && c < coords.length ? coords[c] : [0, 0]];
                if (vmap in vertexMap) {} else {
...
Resource.parse.fromSTL = function (res) {
    ...
    for (var i = 0; i < nTri; i++) {
        ...
        var triBuff = buffer.slice(84 + i * 50, 84 + (i + 1) * 50 - 2);

        var triFloatBuff = new Float32Array(triBuff);

        var n = [triFloatBuff[0], triFloatBuff[1], triFloatBuff[2]];
        var triangle = [];
        for (var j = 3; j < 12; j += 3) {
            var p = [triFloatBuff[j], triFloatBuff[j + 1], triFloatBuff[j + 2]];
            if (vertexmap[p]) {} else {
                vertices.push(p);
...
Resource.parse.from3DS._edit3ds = function (buff, coff,
...
Resource.parse.from3DS._edit_object = function (buff, coff,
// buffer, chunk offset, length
clng) {
    var res = {};
    var off = coff + 6;
...
Resource.parse.from3DS._obj_trimesh = function (buff, coff,
// buffer, chunk offset, length
clng) {
    var res = {};
    var off = coff + 6;
...
Resource.parse.from3DS._tri_vertexl = function (buff, coff,
// buffer, chunk offset, length
clng) {
    var res = [];
    var off = coff + 6;
...
Resource.parse.from3DS._tri_facel1 = function (buff, coff,
// buffer, chunk offset, length
clng) {
    var res = [];
    var off = coff + 6;
...
Resource.parse.from3DS._tri_mappingcoors = function (buff, coff,
// buffer, chunk offset, length
clng) {
    var res = [];
    var off = coff + 6;
...
Resource.parse.from3DS._tri_local = function (buff, coff,
// buffer, chunk offset, length
clng) {
    var res = {};
    var off = coff + 6;
