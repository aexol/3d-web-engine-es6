...
import {Buffer,Indexer} from './buffer';
...
export class Mesh extends MObject {
     ...
     */
    static plane(detailX, detailY, options) {
    ...
    static sphere(detail, options) {
        ...
        mesh.vertices = indexer.unique.map(v => v.vertex);
        if (mesh.coords)
            mesh.coords = indexer.unique.map(v => v.coord);
        if (mesh.normals)
     ...
     */
    static obj(url, callback, readmode) {
        Resource.load(url, e => {
    ...
    static fromData(data1, callback, opts) {
      var meshes = {}
      var count = 0
      for (var m in data1) {
          count += 1
          var data = data1[m];
          let options = {"coords": true};
          if (!data.coords)
              options.coords = false;
          if (!data.normals)
              options.normals = false;
